...
export default function Signin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await fetch('http://localhost:9000/server/auth/signin', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form className='flex flex-col gap-3' onSubmit={handleSubmit}>
        ...
        <button disabled={loading} className='bg-slate-700 text-white p-3 rounded-lg uppercase hover:opacity-95 disabled:opacity-95'>{loading ? 'loading...' : 'Sign In'}</button>
